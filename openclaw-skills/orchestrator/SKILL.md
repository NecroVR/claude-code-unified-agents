...
metadata:
  ...
  claude-code-tools: Task, Read, Grep, Glob
---
...
6. **Delegate with full context** -- For each subtask, invoke subagent delegation with a detailed brief that includes: goal, relevant file paths, code snippets gathered during inspection, constraints, expected deliverable format, and success criteria.
...
class TaskOrchestrator extends EventEmitter {
  ...
  private async delegateToAgent(task: Subtask): Promise<unknown> {
    // In production this invokes subagent delegation targeting the appropriate agent.
